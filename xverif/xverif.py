...
def deterministic(
    ...
    if forecast_type == "continuous":
        ...
        print(
            "- Elapsed time for forecast continuous deterministic verification: {:.2f} minutes.".format(
                (time.time() - t_i) / 60
            )
        )
        ...
        print(
            "- Elapsed time for forecast categorical deterministic verification: {:.2f} minutes.".format(
                (time.time() - t_i) / 60
            )
        )
    else:
        ...
        ds_skill = _spatial_metrics_xarray(
            pred=pred, obs=obs, dim=aggregating_dim, thr=thr, win_size=win_size
        )
        print(
            "- Elapsed time for forecast spatial deterministic verification: {:.2f} minutes.".format(
                (time.time() - t_i) / 60
            )
        )
